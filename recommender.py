...
class RecommendationEngine:
    ...
    def generate_explanation(self, item_name: str, user_ratings: Dict[str, int], 
        ...
        for rated_item, rating in user_ratings.items():
            if rating >= 4:  # User liked this item
                liked_items.append((rated_item, rating))
                similarity = self.calculate_content_similarity(item_name, rated_item)
                if similarity > 0.2:  # Lower threshold for better explanations
        ...
        if similar_items:
            ...
            explanations.append(f"We recommend this because you loved '{top_similar[0]}' ({top_similar[2]}/5 stars). This item shares {top_similar[1]:.0%} similarity with it")
            
            # Add more similar items if available
            if len(similar_items) > 1:
                other_similar = similar_items[1]
                explanations.append(f"It's also similar to '{other_similar[0]}' which you rated {other_similar[2]}/5 stars")
        elif liked_items:
            # If no similar items found, mention genre or general preference
            top_liked = max(liked_items, key=lambda x: x[1])
            explanations.append(f"We recommend this based on your preference for items like '{top_liked[0]}' which you rated {top_liked[1]}/5 stars")
        
        # Genre-specific explanation
        if 'genre' in item_data and pd.notna(item_data['genre']):
            ...
            for rated_item, rating in user_ratings.items():
                ...
                if not rated_data.empty and 'genre' in rated_data.columns:
                    ...
                    if pd.notna(rated_genre) and rated_genre == item_data['genre'] and rating >= 4:
                        genre_items.append((rated_item, rating))
            
            if genre_items and not similar_items:  # Only add if we haven't mentioned specific items
                avg_genre_rating = np.mean([r for _, r in genre_items])
                if len(genre_items) == 1:
                    explanations.append(f"You enjoyed the {item_data['genre']} item '{genre_items[0][0]}' ({genre_items[0][1]}/5 stars)")
                else:
                    explanations.append(f"You consistently rate {item_data['genre']} items highly (average {avg_genre_rating:.1f}/5 stars across {len(genre_items)} items)")
        
        # Quality indicator
        if popularity_score >= 4.5:
            explanations.append(f"Plus, this is a highly-rated item with {popularity_score:.1f}/5 stars from other users")
        elif popularity_score >= 4.0:
            explanations.append(f"It also has good ratings ({popularity_score:.1f}/5 stars) from other users")
        
        # If no good explanations found, provide fallback
        if not explanations:
            if liked_items:
                explanations.append(f"Based on your rating patterns, particularly your {max(liked_items, key=lambda x: x[1])[1]}/5 rating for '{max(liked_items, key=lambda x: x[1])[0]}', we think you might enjoy this")
            else:
                explanations.append(f"This is a popular item in the {item_data['category']} category with {popularity_score:.1f}/5 stars")
    ...
    def get_recommendations(self, user_ratings: Dict[str, int], category: str, 
        ...
        category_items = self.items_df[self.items_df['category'] == category].copy()
        rated_item_names = list(user_ratings.keys())
        unrated_items = category_items[~category_items['name'].isin(rated_item_names)].copy()
        # Ensure unrated_items is a DataFrame
        if not isinstance(unrated_items, pd.DataFrame):
            return []
